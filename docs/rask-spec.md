...
- 終了デバイス: `0x00100000`。word ストアのみを定義済みアクセスとする。値が `0x00005555` なら正常終了する。値の下位 16 bit が `0x3333` なら失敗終了し、終了コードは値の上位 16 bit である。それ以外の値の word ストア、および byte / half ストアは、7 章の `mmio-violation` である。

各デバイスレジスタは、それを含むデバイス領域に属する。

- UART デバイス領域: `0x10000000` から `0x10000fff`
- 終了デバイス領域: `0x00100000` から `0x00100fff`

デバイス領域内では、定義済みデバイスレジスタへの定義済みの値・幅のストアのみが正常なアクセスであり、それ以外のアクセス、つまりデバイスレジスタへのロード、未定義レジスタ番地へのアクセス、定義外の値・幅のストアは、すべて 7 章の `mmio-violation` である。デバイス領域外かつ RAM 外のアドレスへのアクセスは 7 章の unmapped エラーである。

アドレス値は QEMU virt マシンの慣習と互換だが、本仕様は QEMU に依存しない。

### 2.3 Pipeline State

IF stage slot と、IF/ID、ID/EX、EX/MEM、MEM/WB の 4 ラッチを定義する。IF stage slot は、その cycle に IF で保持されている dynamic instruction を表す可視化・trace 用の状態であり、stall 中は PC と同じく凍結される。ラッチフィールドは以下の 3 規則から導出される。

- R1 因果: あるステージで生産された情報は、それを消費するステージまでの間のすべてのラッチに載って運ばれる。
- R2 共通メタデータ: `bubble`、`seqId`、`pc`、`errorKind` は IF で生産され、retire 処理、可視化、retire log、error report が消費するため、IF stage slot と R1 により全ラッチに載る。
...
- `seqId`: IF stage slot に入った時点で採番される 0-origin の dynamic instruction ID。フラッシュされた命令の ID は再利用しない
...
- 未マップ・ミスアラインアドレスへのロード/ストア、および 2.2 のデバイス領域内の定義外アクセス。デバイスレジスタへのロードを含む。MEM で検出する。
...
- MMIO 領域に対しては、定義済みデバイスレジスタへの定義済みの値・幅のストア以外のアクセスを行わない。QEMU の UART には読み出し可能なステータスレジスタが実在し、また QEMU の終了デバイス sifive_test は定義外の値のストアを無視して続行するなど、oracle と `rask` の挙動が割れるためである。この制約に違反したアクセスの挙動は `rask` のエラー条件としてのみ定まり、oracle 比較の対象外である。
- ストアした領域からフェッチしない。
...
program が exit または error termination に到達した場合、ログの最終行は以下のいずれかの terminal record である。program が exit device store や error を伴わず pipeline drain で停止した場合、terminal record は現れない。
...
- `.`: パイプラインに存在しない。フェッチ前。
- `F`: そのサイクルに IF stage slot で保持されていた。stall により IF が凍結される場合は `F` が繰り返される。
...
S1 と S2 の `D` の繰り返しがデータハザードである。依存元が ID/EX・EX/MEM・MEM/WB にいる 3 サイクルを待ち、4 サイクル目に成功する。S2 と S3 の `F` の繰り返しは、下流 stall により IF stage slot が保持されていることを示す。S2 の `X`、つまりサイクル 11 で分岐が解決し、サイクル 11 末の state update で PC 更新と IF/ID・ID/EX のフラッシュが適用される。S3 は `D` を最後にサイクル 11 で消え、サイクル 11 中にフェッチされた S4 は `redirect` により受理されず `F` のみを残す。S5 はサイクル 12 にターゲットから改めてフェッチされる。EX が空になるのはサイクル 12 と 13 の 2 サイクルであり、6.2 の 2 バブルと整合する。
