...
CPU Pipeline Playground は、RV32I CPU `rask` の 5 段 in-order pipeline を観察するための密度の高いワークベンチである。この文書は、現行の assembler / simulator / UI / tests / oracle を `docs/rask-spec.md` に定義された第 1 段階仕様へ移行するためのロードマップを定義する。
...
`rask` は独自 ISA ではなく RV32I を実行する CPU である。ここでの RV32I 対応は、実バイナリ、ABI、OS、privileged architecture まで含む完全な環境互換ではなく、`rask` 仕様が定める RV32I CPU としての機能・タイミング・検証契約に実装を揃えることを指す。
...
`fence`、`ebreak`、`ecall` は範囲外ではない。第 1 段階では `docs/rask-spec.md` の 3 章に従い、`fence` は NOP、`ebreak` はリタイア時 pause、`ecall` はエラー条件として扱う。
...
- 現行実装の挙動が `rask` 仕様と異なる場合、後方互換ではなく仕様準拠へ修正する。
