...
参照 ISA は初期設計では RV32I に固定する。現在の simulator は JavaScript number と 32-bit bitwise 演算を使っており、既存命令セットも RV32I に自然に対応する。将来 RV64 や追加拡張を扱う余地は残すが、初期の signature と comparator は RV32I / ILP32 を前提に単純に保つ。
...
比較対象は、現在の simulator が責務として持つ architectural state を原則にする。現時点では整数レジスタ、観測対象メモリ、制御フロー由来の logical PC 情報や `jal` link value が中心になる。QEMU の生 PC は harness の起動・終了コードまで進むため、全テストでそのまま比較しない。分岐やジャンプの検証では、到達ラベル、logical PC、link value など、テスト本体相対で意味のある値を signature に含められるようにする。
...
現行命令セットを守ることは目的ではない。現行命令だけに固執するとテスト基盤が不自然になり、品質が下がる場合は、必要な RV32I 命令や assembler 機能を追加してよい。ただし、追加は QEMU 参照テストの読みやすさ、共通 `.asm` 断片の自然さ、将来の RISC-V pipeline viewer 化に資する場合に限る。
...
- 現行命令セットがテストの自然さや品質の足かせになる場合、必要な RV32I 命令や assembler 機能を追加できる余地が設計上残っている。
