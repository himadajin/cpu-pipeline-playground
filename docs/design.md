...
中心価値は、ユーザーが複数の小さなプログラムを管理し、編集し、実行し、pipeline timeline・hazard・stall・flush・レジスタ/メモリ更新の変化を即座に見られることにある。core と画面に見える状態は `docs/rask-spec.md` に定義された `rask` 仕様に従う。
...
CPU モデルは `rask` 仕様に従う。UI は IF / ID / EX / MEM / WB、依存による stall、branch flush、commit、レジスタ更新、メモリ更新、エラーを観察できるようにする。
...
右側の Inspector は、timeline の命令セルやイベントをクリックしたときに、その cycle・その命令の説明、hazard 理由、flush 理由、commit、memory diff、error を表示する領域にする。Registers tab は `x0` から `x31` まで順番固定で表示し、現在 cycle で changed な register だけを控えめに強調する。Memory tab は memory state と current cycle の memory diff を表示する。
...
UI 上の説明は短いラベル、tooltip、inspector のイベント説明に寄せる。エラー、hazard、stall、flush の理由は、ユーザーが該当セルやイベントを選んだときに理解できる粒度で表示する。
